...
export default function AdminDashboardPage() {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/rates`, {
      credentials: "include",
    });
...
const handleSaveRate = async (rate: Rate) => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/rates/${rate.currency}`, {
      method: "PATCH",
      credentials: "include",
...
const handleDeleteRate = async (currency: string) => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/rates/${currency}`, {
      method: "DELETE",
      credentials: "include",
    });
 ...
 const fetchConfig = async () => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/config`, {
      credentials: "include", // 🔐 Requiere para Clerk en subdominios
    });
    const data = await res.json();
...
const handleSaveConfig = async () => {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/config`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include", // 🔐 Incluye cookies de Clerk
      body: JSON.stringify({
